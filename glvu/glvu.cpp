...
void GLVU::Keyboard(unsigned char Key, int x, int y){
    ...
    switch (Key) {
        case 27: EscapeHandler(-1);
            break;

        case 'z': MainMenuHandler(0);
            break;
        case 'h': MainMenuHandler(1);
            break;
        case 'x': MainMenuHandler(2);
            break;
        case 'c': MainMenuHandler(3);
            break;
        case 'v': MainMenuHandler(4);
            break;

        // case '=': vuOptionsMenuHandler(1); break;
        // case 'o': vuOptionsMenuHandler(2); break;
        case 'r': vuOptionsMenuHandler(3);
            break;
...
void GLVU::vuOptionsMenuHandler(int value){
    ...
    switch (value) {
        ...
        case 4: {
            GLVU * g = GetGLVU();
            if (g->GetInertiaEnabled()) { g->SetInertiaEnabled(0); g->SetInertiaOn(0); } else {
...
void GLVU::Motion(int x, int y){
    ...
    if (LeftButtonDown) {
        ...
        switch (GetWorldNavMode()) {
            // -------  WORLD NAVIGATION -------
            case NAV_MODE_TRACKBALL:
                if (CtrlPressed) { TrackBallX(OldX, NewX, WW); DriveY(OldY, NewY, WH); } else {
            ...
            case NAV_MODE_HYPERBALL:
                ...
                break;
            case NAV_MODE_DRIVE:
                if (CtrlPressed) { TranslateX(NewX, OldX, WW); TranslateY(NewY, OldY, WH); } else if (AltPressed) {
                    LookX(OldX, NewX, WW);
                    LookY(OldY, NewY, WH);
            ...
            case NAV_MODE_TRANSLATE:
                ...
                break;
            case NAV_MODE_LOOK:
                if (CtrlPressed) { TranslateX(OldX, NewX, WW); TranslateY(OldY, NewY, WH); } else if (AltPressed) {
                    LookX(OldX, NewX, WW);
                    DriveY(OldY, NewY, WH);
